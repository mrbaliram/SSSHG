...
<x-app-layout>
    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ 'Update record which is paid for society member' }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('contribution_payment.update', $results->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT') 
                            ...
                            <div class="relative z-0 w-full mb-6 group">
                                ...
                                <select id="society_member_id" name="society_member_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required>
                                    ...
                                    @foreach($societyMembersResults as $societyMemberData)
                                        <option value="{{ $societyMemberData->id }}"  {{   $societyMemberData->id == $results->society_member_id ? 'Selected' : '' }}> {{ $societyMemberData->memberName }} [{{ $societyMemberData->societyName }}]</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="relative z-0 w-full mb-6 group">
                        ...
                        <div class="grid md:grid-cols-2 md:gap-6">
                            ...
                            <div class="relative z-0 w-full mb-6 group">
                                ...
                                <input id="late_fine" type="number" name="late_fine" value="{{$results->late_fine}}" class="border rounded-md w-full py-2 px-3 text-gray-700">
                            </div>
                        </div>
                        ...
                        <div class="grid md:grid-cols-2 md:gap-6">
                            <div class="relative z-0 w-full mb-6 group">
                                ...
                                <?php
                                    ...
                                    $dateArr = explode("-", $results->pay_for_month_year);
                                    $currentMoth = $dateArr[0];
