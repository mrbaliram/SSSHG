...
<x-app-layout>
    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ $results ? 'Update Member Type' : 'Create Member Type' }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ $results ? route('member_type.update', $results->id) : route('member_type.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @if ($results)
                    @method('PUT')
                     @endif
                        <div class="mb-4">
                            <label for="name" class="block text-gray-600 font-medium">Member type<span style="color:red"> *</span></label>
                            <input type="text" name="name" id="name" value="{{old('name', $results->name ?? '')}}" class="border rounded-md w-full py-2 px-3 text-gray-700" required>
                        </div>

                        <div class="mb-4">
                            <label for="code" class="block text-gray-600 font-medium">Member type code<span style="color:red"> *</span></label>
                            <input type="text" name="code" id="code" value="{{old('code', $results->code ?? '')}}" class="border rounded-md w-full py-2 px-3 text-gray-700" required>
                        </div>
                        ...
                        <div class="mb-4" align="center">
                            ...
                            <a href="{{ route('member_type.index') }}" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">{{ __('Go Back') }}</a>
