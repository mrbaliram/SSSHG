            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="mb-4 flex justify-end">
                        <form action="{{ route('member.index') }}" method="POST" class="inline">
                            @csrf
                            @method('GET')
                        ...
                        @if(Auth::user()->type == 'cashier')
                            <a href="{{ route('member.create') }}" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800 flex">
                                ...
                                @foreach($results as $data)
                                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                        ...
                                        <td scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                            {{ $data->name }} @if ($data->name != $data->guardian) ({{ $data->guardian }}) @endif
                                        </td>
                                        <td scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        ...
                                        <td scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                            {{ $data->address1 }} @if($data->address2 != '') ({{ $data->address2 }}) @endif
                                        </td>
                                        <td scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        ...
                                        <td class="px-6 py-4 flex">
                                            @if(Auth::user()->type == 'cashier')
                                                <!-- Edit icon  -->
                                                <a href="{{ route('member.edit', $data->id) }}" class="text-blue-600 hover:underline" title="Edit / Update">
                                                    <svg class="h-6 w-6 text-blue-600"  viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">  <path stroke="none" d="M0 0h24v24H0z"/>  <path d="M9 7 h-3a2 2 0 0 0 -2 2v9a2 2 0 0 0 2 2h9a2 2 0 0 0 2 -2v-3" />  <path d="M9 15h3l8.5 -8.5a1.5 1.5 0 0 0 -3 -3l-8.5 8.5v3" />  <line x1="16" y1="5" x2="19" y2="8" /></svg>
                                                </a>&nbsp;&nbsp;

                                                <!-- Delete icon button  -->
                                                <form action="{{ route('member.destroy', $data->id) }}" method="POST" class="inline">
                                                    @csrf
                                                    @method('DELETE')
                                            ...
                                            @endif
                                            <!-- Show details button  -->
                                            <a title="show details" href="{{ route('member.show', $data->id) }}" class="text-blue-600 hover:underline">
