        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('loan_payment.update', $results->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT') 
                            ...
                            <div class="relative z-0 w-full mb-6 group">
                                ...
                                <select id="loan_account_id" name="loan_account_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required>
                                    ...
                                    @foreach($loanAccountResult as $data)
                                        <option value="{{ $data->id }}" data-fullAmount="{{$data->full_amount}}" {{ $data->id == $results->loan_account_id ? 'Selected' : '' }}> {{ $data->memberName }} [{{ $data->societyCode }}] Amount ({{$data->full_amount}})</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="relative z-0 w-full mb-6 group">
                                <label for="pay_date" class="block text-gray-600 font-medium">Pay Date<span style="color:red"> *</span></label>
                                <input type="date" name="pay_date" id="pay_date" value="{{date('Y-m-d', strtotime($results->pay_date))}}" class="border rounded-md w-full py-2 px-3 text-gray-700">
                            </div>
                        </div>
                        
                        <div class="grid md:grid-cols-2 md:gap-6">
                            <div class="relative z-0 w-full mb-6 group">
                                <label for="paid_amount" class="block text-gray-600 font-medium">Paid Amount</span></label>
                                <input id="paid_amount" type="number" name="paid_amount" value="{{$results->paid_amount}}" class="border rounded-md w-full py-2 px-3 text-gray-700">
                            </div>
                            <div class="relative z-0 w-full mb-6 group">
                                
                                <label for="intrest_amount" class="block text-gray-600 font-medium">Intrest Amount</label>
                                <input id="intrest_amount" type="number" name="intrest_amount" value="{{$results->intrest_amount}}" class="border rounded-md w-full py-2 px-3 text-gray-700">
